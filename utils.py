...
def parse_dia_hora(texto: str):
    """
    Acepta:
      - hoy 15:00 / mañana 12:30 / hoy 15 / mañana 9
      - (este|próximo|el)? viernes (a las)? 14(:30)?
      - lunes 15:00 / miércoles 9
    ...
    # 2) (este|próximo|el)? <dia_semana> (a las)? HH(:MM)?
    m = re.match(
        r"^(?:este|pr[oó]ximo|el)?\s*(lunes|martes|miercoles|miércoles|jueves|viernes|sabado|sábado|domingo)"
    ...
    # 4) el <día_mes> (a las)? HH(:MM)?   -> usa mes actual; si pasó, siguiente mes
            ...
            if session["paso"] > 1:
                if session["paso"] == 2:
                    ...
                    return ("Has vuelto atrás ↩️. Por favor, indícanos *día y hora*.\n"
                            "Ejemplos: 'martes 15:00', '13/08 15:00', 'mañana 12:30', 'este viernes a las 14', 'el 20 a las 13'.")
                elif session["paso"] == 4:
                    session["paso"] = 3
